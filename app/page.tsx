...
export default function Home() {
  ...
  useEffect(() => {
    const savedTotals = window.localStorage.getItem(STORAGE_KEY);
    ...
    try {
      const parsedTotals = JSON.parse(savedTotals) as Record<string, string>;
  ...
  return (
    <div className="flex flex-col items-center justify-center bg-zinc-50 font-sans dark:bg-black">
      ...
      {mealPlans.map((plan) => (
        <div key={plan.title} className="flex w-full flex-col items-center justify-center px-16 py-8 bg-white dark:bg-black">
                ...
                <tr key={row.totalKey}>
                  ...
                  <td className="border border-gray-300 dark:border-gray-700 px-2 py-2">
                    <input
                      type="number"
                      min="0"
                      value={totals[row.totalKey] ?? ""}
                      onChange={(event) => handleTotalChange(row.totalKey, event.target.value)}
